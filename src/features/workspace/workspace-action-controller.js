...
function installWorkspaceSwipeBack(ctx){var el=qs(ctx,'workspace-section');if(!el||el.dataset.swipeBackReady==='1')return false;el.dataset.swipeBackReady='1';var sx=0,sy=0,dx=0,tracking=false,claimed=false;el.addEventListener('touchstart',function(e){var t=e.touches&&e.touches[0];if(!t)return;if(!ctx.workspaceState||ctx.workspaceState.panel==='home')return;sx=t.clientX;sy=t.clientY;dx=0;claimed=false;tracking=sx<96},{passive:true});el.addEventListener('touchmove',function(e){if(!tracking)return;var t=e.touches&&e.touches[0];if(!t)return;dx=t.clientX-sx;var dy=Math.abs(t.clientY-sy);if(dx>16&&dx>dy*1.35){claimed=true;if(e.preventDefault)e.preventDefault();if(e.stopPropagation)e.stopPropagation();}},{passive:false});el.addEventListener('touchend',function(e){if(!tracking)return;tracking=false;if(dx>82){if(e&&e.stopPropagation)e.stopPropagation();if(e&&e.preventDefault)e.preventDefault();workspaceBack(ctx);}claimed=false;},{passive:false});return true;}
