...
function createAdapter(deps){
  ...
  function displayChord(ch,song,mode){var st=state();song=song||{};var sourceKey=song.chartKey||song.originalKey||song.sourceKey||song.key||'C';var key=song.renderKey||song.targetKey||song.displayKey||st.displayKey||song.key||sourceKey||'C';var t=transposeChord(ch,sourceKey,key);return mode==='nns'?chordToNns(t,key):t}
