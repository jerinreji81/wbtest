...
function syncSongMenuToggle(ctx,action,on){
  var id=action==='flats'?'mi-flats':(action==='dark'?'mi-dark':(action==='highlight'?'mi-highlight-chords':''));
  var b=id?qs(ctx,id):null;if(!b)return;
  b.classList.toggle('on',!!on);
  var sw=b.querySelector('.wbsm-toggle');if(sw)sw.classList.toggle('on',!!on);
}
