...
function settingShortLabel(key,val,controller){var label=settingLabel(key,val,controller);if(key==='defaultPdfPreset'){var map={stage:'Stage',rehearsal:'Rehearsal',compact:'Compact',lyrics:'Lyrics'};return map[val]||String(label||'').split(/\s+/)[0]||label;}return label;}
function choiceSub(key,settings,controller){
  ...
  if(key==='defaultPdfPreset')return 'Open export on '+settingShortLabel('defaultPdfPreset',settings.defaultPdfPreset,controller)+' preset';
