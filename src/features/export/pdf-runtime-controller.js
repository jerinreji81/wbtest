...
function songSourceKey(song,fallback){song=song||{};return song.chartKey||song.originalKey||song.sourceKey||song.key||fallback||'C'}
...
function exportDisplayChord(ch,song,key,mode){var source=songSourceKey(song,(song&&song.key)||'C'),target=key||((song&&song.displayKey)||source);var t=transposeChord(ch,source,target);return mode==='nns'?chordToNns(t,target):t}
