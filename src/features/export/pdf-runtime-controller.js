...
function wbPdfTransposeLine(line,fromKey,toKey,opts){var s=String(line||'');try{if(typeof getST==='function'&&typeof procLine==='function')return procLine(s,getST(fromKey,toKey),toKey,opts&&opts.exportContent==='nns',!!settings.useFlatsDefault)}catch(e){}if(/\[[^\]]+\]/.test(s))return s.replace(/\[([^\]]+)\]/g,function(_,c){return '['+wbPdfTransposeChord(c,fromKey,toKey)+']'});return s.replace(/(^|\s)([A-G](?:#|b)?[^\s]*)/g,function(m,pre,c){return pre+wbPdfTransposeChord(c,fromKey,toKey)})}
function wbPdfInlinePair(raw,songKey,printKey,opts){var st=0,usedProc=false;try{if(typeof getST==='function')st=getST(songKey,printKey)}catch(e){}raw=String(raw||'');try{if(typeof procLine==='function'){raw=procLine(raw,st,printKey,opts&&opts.exportContent==='nns',!!(window.settings&&settings.useFlatsDefault)||!!window.useFlats);usedProc=true}}catch(e){usedProc=false}var lyric='',chords=[],last=0,re=/\[([^\]]+)\]/g,m;while((m=re.exec(raw))){var before=raw.slice(last,m.index);lyric+=before;var chord=m[1];if(!usedProc)chord=wbPdfTransposeChord(chord,songKey,printKey);try{if(!usedProc&&opts&&opts.exportContent==='nns'&&typeof chordToNns==='function')chord=chordToNns(chord,printKey)}catch(e){}wbPdfPlaceChord(chords,lyric.length,chord);last=re.lastIndex}lyric+=raw.slice(last);return {ch:Array.from({length:chords.length},function(_,i){return chords[i]||' '}).join('').replace(/\s+$/,''),ly:wbPdfSafe(lyric)}}
function wbPdfChordLyricPair(chordLine,lyricLine,songKey,printKey,opts){var st=0,usedProc=false;try{if(typeof getST==='function')st=getST(songKey,printKey)}catch(e){}var ch=String(chordLine||'');try{if(typeof procLine==='function'){ch=procLine(ch,st,printKey,opts&&opts.exportContent==='nns',!!(window.settings&&settings.useFlatsDefault)||!!window.useFlats);usedProc=true}}catch(e){usedProc=false}if(!usedProc)ch=wbPdfTransposeLine(ch,songKey,printKey,opts);return {ch:wbPdfSafe(ch).replace(/[ 	]+$/,''),ly:wbPdfSafe(lyricLine||'')}}
...
function wbPdfWrapPair(pair,max){var ch=wbPdfSafe(pair&&pair.ch||''),ly=wbPdfSafe(pair&&pair.ly||''),len=Math.max(ch.length,ly.length);ch=ch.padEnd(len,' ');ly=ly.padEnd(len,' ');var out=[],a=ch,b=ly;while(Math.max(a.length,b.length)>max){var cut=b.lastIndexOf(' ',max);if(cut<Math.floor(max*.50))cut=max;out.push({ch:a.slice(0,cut).replace(/\s+$/,''),ly:b.slice(0,cut).replace(/\s+$/,'')});a=a.slice(cut);b=b.slice(cut);var lead=(b.match(/^\s+/)||[''])[0].length;if(lead){a=a.slice(lead);b=b.slice(lead)}}if(a.trim()||b.trim())out.push({ch:a.replace(/\s+$/,''),ly:b.replace(/\s+$/,'')});return out}
...
function wbPdfSvgText(s,x,y,size,font,weight,color){s=wbPdfSafe(s);if(!s.length)return '';return '<text x="'+wbPdfNum(x)+'" y="'+wbPdfNum(y)+'" font-family="'+wbPdfEsc(font||'-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif')+'" font-size="'+wbPdfNum(size||10)+'" font-weight="'+wbPdfEsc(weight||400)+'" fill="'+wbPdfEsc(color||'#101820')+'">'+wbPdfEsc(s)+'</text>'}
