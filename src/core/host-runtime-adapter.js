...
function copyCurrentLyrics(){
  ...
  var txt=String(s.chart||'').replace(/\[[^\]]+\]/g,'').replace(/\n{3,}/g,'\n\n').trim();
...
function keyChoiceButtons(active,includeOriginal,set,item,song){var opts=[],isSong=!!item; if(isSong)opts.push(['inherit','Follow set default',resolveSetKey(set,song)]);opts.push(['original','Original key',(song&&song.key)||firstSongKey(set)||'G']);opts.push(['default','App default key',defaultKeyFor(song,set)]);NOTES.forEach(function(n){opts.push([n,n,'']);});return opts.map(function(o){var wide=(o[0]==='inherit'||o[0]==='original'||o[0]==='default')?' wide':'';return '<button class="key-option-btn'+wide+' '+(String(o[0])===String(active||'original')?'active':'')+'" data-set-key="'+esc(o[0])+'" type="button">'+esc(o[1])+(o[2]?'<span class="key-option-sub">'+esc(o[2])+'</span>':'')+'</button>'}).join('')}
