...
function qa3ConfirmDeletePersonalSet(id){var doomed=personalSets.find(function(x){return x.id===id});return appConfirm('Delete this set list?',{title:'Delete set list',detail:doomed?('This set list has '+countLabel(doomed)+'. You can undo immediately after deleting.'):'You can undo immediately after deleting.',confirmText:'Delete',danger:true}).then(function(ok){if(!ok)return;var snap=dataSafetySnapshot('delete set list');personalSets=personalSets.filter(function(x){return x.id!==id});persistSets();renderPersonalHome();renderSettings();offerDataUndo('Set deleted',snap)});}
function qa3InstallSetActionFallbacks(){
  if(document.body&&document.body.dataset.qa3SetActionFallbacks==='1')return;
  if(document.body)document.body.dataset.qa3SetActionFallbacks='1';
  document.addEventListener('click',function(e){
    var t=e.target;if(!t||!t.closest)return;
    var pd=t.closest('[data-delete-set]');
    if(pd){e.preventDefault();e.stopPropagation();qa3ConfirmDeletePersonalSet(pd.dataset.deleteSet);return;}
    var wd=t.closest('[data-workspace-delete]');
    if(wd){e.preventDefault();e.stopPropagation();if(window.WBWorkspaceActionController&&window.WBWorkspaceActionController.deleteWorkspaceSet){window.WBWorkspaceActionController.deleteWorkspaceSet(workspaceActionCtx(),wd.dataset.workspaceDelete)}return;}
    var clear=t.closest('#clear-set,.sl-clear-btn[data-workspace-clear]');
    if(clear){e.preventDefault();e.stopPropagation();if(clear.dataset.workspaceClear){var ws=(workspaceState.sets||[]).find(function(x){return x.id===clear.dataset.workspaceClear});if(ws){appConfirm('Clear all items from this Workspace set?',{title:'Clear Workspace set',detail:'This will remove '+workspaceCountLabel(ws)+' from the Workspace set. You can undo immediately after clearing.',confirmText:'Clear',danger:true}).then(function(ok){if(!ok)return;var snap=dataSafetySnapshot('clear Workspace set');if(WorkspaceSetController&&WorkspaceSetController.clearSet)WorkspaceSetController.clearSet(ws);else{ws.items=[];workspaceSetCounts(ws)}commitSetMutation('workspace',ws,{action:'clearSet',refresh:['workspaceDetail','workspaceLists']});offerDataUndo('Workspace set cleared',snap)})}}else clearCurrentSet();return;}
    var wr=t.closest('[data-workspace-remove]');
    if(wr){e.preventDefault();e.stopPropagation();var wsSet=activeWorkspaceSet&&activeWorkspaceSet();if(wsSet){appConfirm('Remove this item from the Workspace set?',{title:'Remove item',detail:'This only removes the item from this Workspace set. You can undo immediately after removing.',confirmText:'Remove',danger:true}).then(function(ok){if(!ok)return;var snap=dataSafetySnapshot('remove Workspace item');if(WorkspaceSetController&&WorkspaceSetController.removeItem)WorkspaceSetController.removeItem(wsSet,parseInt(wr.dataset.workspaceRemove,10));else{wsSet.items.splice(parseInt(wr.dataset.workspaceRemove,10),1);workspaceSetCounts(wsSet)}commitSetMutation('workspace',wsSet,{action:'removeItem',refresh:['workspaceDetail','workspaceLists']});offerDataUndo('Removed from Workspace set',snap)})}return;}
    var pr=t.closest('[data-remove-index]');
    if(pr){e.preventDefault();e.stopPropagation();removeSetItem(parseInt(pr.dataset.removeIndex,10));return;}
  },true);
}
