...
function ensurePadNowPlayingPill(){var el=qs('pad-now-playing-pill');if(el)return el;el=document.createElement('div');el.id='pad-now-playing-pill';el.className='pad-now-playing-pill';el.hidden=true;el.innerHTML='<div class="pad-now-copy"><strong data-pad-now-title>Peaceful · C</strong><span data-pad-now-sub></span></div><button type="button" data-pad-stop-global aria-label="Stop pads"><span class="pad-stop-icon" aria-hidden="true"></span></button>';document.body.appendChild(el);el.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('[data-pad-stop-global]');if(!b)return;e.preventDefault();if(window.WorshipBasePad&&window.WorshipBasePad.stop)window.WorshipBasePad.stop();updatePadNowPlayingPill();renderPadSheet();updateSongTopActionStates();});return el}
