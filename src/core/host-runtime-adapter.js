...
function normalizeUgImportedSong(raw){
  ...
  var chartKey=inferKeyFromChordProgression(chart)||normalizeKeyName(raw.chartKey||raw.sourceKey||raw.originalKey)||detectImportKey(chart)||normalizeKeyName(raw.key||raw.scale)||'C';
  var key=chartKey||'C';
...
function updatePadNowPlayingPill(){var el=qs('pad-now-playing-pill'),wb=window.WorshipBasePad;if(!el||!wb||!wb.state){if(el)el.hidden=true;return}var st=wb.state||{},on=!!st.on;el.hidden=!on;el.classList.toggle('show',on);var title=el.querySelector('[data-pad-now-title]'),sub=el.querySelector('[data-pad-now-sub]');if(title)title.textContent=on?'Pad playing':'Pad ready';if(sub)sub.textContent=(String(st.mode||'peaceful').charAt(0).toUpperCase()+String(st.mode||'peaceful').slice(1))+' · '+normalPadKey(st.key||state.displayKey||'C')}
function ensurePadNowPlayingPill(){var el=qs('pad-now-playing-pill');if(el)return el;el=document.createElement('div');el.id='pad-now-playing-pill';el.className='pad-now-playing-pill';el.hidden=true;el.innerHTML='<div class="pad-now-copy"><strong data-pad-now-title>Pad playing</strong><span data-pad-now-sub>Peaceful · C</span></div><button type="button" data-pad-stop-global>Stop</button>';document.body.appendChild(el);el.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('[data-pad-stop-global]');if(!b)return;e.preventDefault();if(window.WorshipBasePad&&window.WorshipBasePad.stop)window.WorshipBasePad.stop();updatePadNowPlayingPill();renderPadSheet();updateSongTopActionStates();});return el}
