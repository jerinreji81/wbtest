...
function normalizeImportedSong(raw,kind){raw=raw||{};var chart=String(raw.chart||raw.lyrics||raw.body||raw.content||raw.text||'').replace(/\r\n?/g,'\n').trim();if(kind==='ug')chart=ugCleanChordSheetText(chart);var explicitKey=raw.key||raw.originalKey||raw.sourceKey||raw.chartKey||raw.detectedKey||raw.scale;var key=normalizeKeyName(explicitKey)||detectImportKey([explicitKey,chart].filter(Boolean).join('\n'))||'C';var sourceKey=normalizeKeyName(raw.chartKey||raw.originalKey||raw.sourceKey||explicitKey)||key||'C';var title=String(raw.title||raw.name||'').trim();var artist=String(raw.artist||raw.author||raw.writer||'').trim();if(!raw._parsed && chart && (!title||!artist)){var parsed=parseImportedText((title?title+'\n':'')+(artist?artist+'\n':'')+chart,kind||'file');title=title||parsed.title;artist=artist||parsed.artist;chart=chart||parsed.chart;key=normalizeKeyName(explicitKey)||parsed.key||key;sourceKey=normalizeKeyName(raw.chartKey||raw.originalKey||raw.sourceKey||explicitKey)||parsed.key||key}var cat=raw.cat||raw.category||(kind==='sop'?'malayalam':'english');return {title:title||'Imported song',artist:artist||'',author:artist||'',code:String(raw.code||'').trim(),key:key||'C',chartKey:sourceKey||key||'C',originalKey:sourceKey||key||'C',sourceKey:sourceKey||key||'C',displayKey:key||'C',cat:cat==='malayalam'?'malayalam':'english',chart:chart||'',bpm:String(raw.bpm||'').trim(),timeSig:String(raw.timeSig||raw.timeSignature||raw.time||'').trim(),spotifyUrl:String(raw.spotifyUrl||raw.spotify||'').trim(),youtubeUrl:String(raw.youtubeUrl||raw.youtube||'').trim(),tabType:String(raw.tabType||raw.type||raw.resultType||'').trim(),rating:String(raw.rating||raw.stars||raw.score||'').trim(),source:'local',importSource:kind||raw.importSource||'file',sourceUrl:String(raw.url||raw.sourceUrl||'').trim(),importId:String(raw.id||raw.importId||'').trim(),selected:raw.selected!==false}}
...
function chordKeyRoot(token){var m=String(token||'').trim().replace(/[|,;]/g,'').match(/^([A-G](?:#|b)?)(.*)$/i);return m?normalizeKeyName(m[1]):''}
function chordTokensFromLineForKey(line){var rx=/[A-G](?:#|b)?(?:maj|major|min|minor|m|aug|dim|sus2|sus4|sus|add(?:2|4|9|11|13)?|no(?:3|5)?|ø|[b#](?:5|9|11|13)|2|4|5|6|7|9|11|13)*(?:\/[A-G](?:#|b)?)?/g;return (String(line||'').match(rx)||[]).slice(0,8)}
function inferKeyFromChordProgression(chart){
  var lines=String(chart||'').replace(/\r\n?/g,'\n').split('\n'),candidates=['C','Db','D','Eb','E','F','F#','G','Ab','A','Bb','B'],maj=[0,2,4,5,7,9,11],min=[0,2,3,5,7,8,10];
  function ni(n){var map={C:0,'C#':1,Db:1,D:2,'D#':3,Eb:3,E:4,F:5,'F#':6,Gb:6,G:7,'G#':8,Ab:8,A:9,'A#':10,Bb:10,B:11};return map[normalizeKeyName(n)]}
  function rootOf(tok){var m=String(tok||'').match(/^([A-G](?:#|b)?)/);return m?normalizeKeyName(m[1]):''}
  var best={key:'',score:-999};
  for(var li=0;li<lines.length;li++){if(!lineLooksChord(lines[li]))continue;var toks=chordTokensFromLineForKey(lines[li]);if(toks.length<2)continue;var roots=toks.map(rootOf).filter(Boolean);if(roots.length<2)continue;
    candidates.forEach(function(k){var ki=ni(k);if(ki==null)return;var score=0;roots.forEach(function(r,idx){var ri=ni(r);if(ri==null)return;var semi=(ri-ki+12)%12;if(maj.indexOf(semi)>=0)score+=3;else if(min.indexOf(semi)>=0)score+=1;else score-=2;if(semi===0)score+=idx===0?4:1;if(semi===7)score+=idx<3?2:1;if(semi===4)score+=1;if(semi===9)score+=1;});if(score>best.score)best={key:k,score:score}});
    if(best.key&&best.score>=roots.length*3)return best.key;
  }
  return best.score>4?best.key:'';
}
function detectImportKey(chart){var m=(chart||'').match(/(?:Key(?:\s+of)?|Original(?:ly)?(?:\s+in)?|Scale)\s*[:\-]?\s*([A-G](?:#|b)?)(?:\s*(m|min|minor))?/i);if(m)return normalizeKeyName(m[1]+(m[2]?'m':''));var inferred=inferKeyFromChordProgression(chart);if(inferred)return inferred;var lines=(chart||'').split('\n');for(var i=0;i<lines.length;i++){if(lineLooksChord(lines[i])){var tok=(lines[i].trim().split(/\s+/)[0]||'');var cm=tok.match(/^([A-G](?:#|b)?)(m|min|minor)?/i);if(cm)return normalizeKeyName(cm[1]+(cm[2]?'m':''))}}return 'C'}
...
function normalizeUgImportedSong(raw){
  ...
  return normalizeImportedSong({id:raw.id||raw.importId,title:title,artist:artist,key:key,originalKey:chartKey,chartKey:chartKey,sourceKey:chartKey,displayKey:key,tabType:raw.tabType||raw.type||raw.resultType||'Chords',rating:raw.rating||raw.stars||raw.score,cat:'english',chart:chart,bpm:raw.bpm||raw.tempo,timeSig:raw.timeSig||raw.timeSignature,url:raw.url||raw.sourceUrl,code:raw.code,_parsed:true},'ug');
}
...
function importResultMetaLine(r){r=r||{};var parts=[];var artist=String(r.artist||r.author||r.writer||'').trim();if(artist)parts.push(artist);var type=String(r.tabType||r.type||r.resultType||'Chords').trim();if(type&&type.toLowerCase()!=='ug')parts.push(type.charAt(0).toUpperCase()+type.slice(1));var rating=String(r.rating||r.stars||r.score||'').trim();if(rating)parts.push('★ '+rating);var bpm=String(r.bpm||r.tempo||'').trim();if(bpm)parts.push(bpm+' BPM');var timeSig=String(r.timeSig||r.timeSignature||'').trim();if(timeSig)parts.push(timeSig);return parts.filter(Boolean).join(' · ')}
function renderImportResults(kind,results,emptyMessage){var box=qs(kind==='sop'?'sop-results':'ug-results');if(!box)return;if(!results||!results.length){box.innerHTML='<div class="preview-empty" style="padding:14px">'+esc(emptyMessage||'No results found.')+'</div>';return}box.innerHTML=results.map(function(r,i){var meta=importResultMetaLine(r);var selected=!!(r&&r.selected);return '<button class="ug-result-item'+(selected?' selected':'')+'" data-import-result="'+i+'" type="button"><span class="pi-copy"><span class="pi-title">'+esc(r.title||'Untitled')+'</span>'+(meta?'<span class="pi-meta">'+esc(meta)+'</span>':'')+'</span><span class="pi-ck" aria-hidden="true">'+(selected?'✓':'')+'</span></button>'}).join('');window[kind==='sop'?'_sopResults':'_ugResults']=results}
function setImportedDraft(kind,draft){draft=(kind==='ug')?normalizeUgImportedSong(draft):normalizeImportedSong(draft,kind);draft._wbRawImportChart=draft.chart;window[kind==='sop'?'_sopImportedSong':'_ugImportedSong']=draft;var body=qs(kind==='sop'?'sop-preview-body':'ug-import-preview');var meta=qs(kind==='sop'?'sop-preview-meta':'ug-preview-meta');if(meta)meta.textContent=[draft.artist||draft.author,'Key of '+(draft.key||'C'),draft.timeSig,draft.bpm?draft.bpm+' BPM':''].filter(Boolean).join(' · ');if(body){body.classList.toggle('chords-highlight',!!settings.highlightChordsDefault);var previewSong=Object.assign({},draft,{key:(draft.key||draft.chartKey||state.displayKey||'C'),displayKey:(draft.displayKey||draft.key||draft.chartKey||'C'),renderKey:(draft.displayKey||draft.key||draft.chartKey||'C')});body.innerHTML='<div class="preview-empty" style="margin-bottom:8px"><strong>'+esc(draft.title)+'</strong>'+(draft.artist?' · '+esc(draft.artist):'')+'</div>'+formatChartHtml(draft._wbRawImportChart||draft.chart,'chords',previewSong)}}
...
var modal=qs('add-song-modal');if(modal&&!modal.dataset.editorReady){modal.dataset.editorReady='1';modal.addEventListener('click',function(e){if(e.target===modal||e.target.id==='add-song-close'||e.target.id==='song-form-cancel'){closeAddSongModal();return}if(e.target.id==='song-form-save'){saveSongFromForm();return}var cat=e.target.closest('[data-song-cat]');if(cat){setSongCategory(cat.dataset.songCat);return}var b=e.target.closest('[data-song-editor-mode]');if(b){setSongEditorPane(b.dataset.songEditorMode);return}var sec=e.target.closest('[data-section-insert]');if(sec){insertSectionLabel(sec.dataset.sectionInsert);return}if(e.target.id==='song-form-paste'){pasteChordsFromClipboard();return}if(e.target.id==='song-form-insert-chord-line'){insertChordLine();return}if(e.target.id==='song-form-duplicate-section'){duplicateCurrentSection();return}if(e.target.id==='song-form-clean-spacing'){normalizeEditorSpacing();return}if(e.target.id==='editor-undo-btn'){editorUndo();return}if(e.target.id==='editor-redo-btn'){editorRedo();return}if(e.target.id==='song-form-delete'){deleteSongFromEditor();return}});modal.addEventListener('input',function(e){if(e.target&&e.target.id==='song-form-chart'){pushEditorHistory()}if(e.target&&/^song-form-/.test(e.target.id||''))updateSongEditorPreview()});modal.addEventListener('change',function(e){if(e.target&&/^song-form-/.test(e.target.id||''))updateSongEditorPreview()})}
var ug=qs('bs-ug-import');if(ug&&!ug.dataset.ready){ug.dataset.ready='1';ug.addEventListener('click',function(e){if(e.target===ug||e.target.id==='ug-import-cancel'){closeImportPages();return}if(e.target.id==='ug-import-search'){searchImport('ug');return}if(e.target.id==='ug-import-url-btn'){importFromUrl('ug');return}if(e.target.id==='ug-import-edit-btn'){editImported('ug');return}if(e.target.id==='ug-import-save-btn'){saveImported('ug');return}var res=e.target.closest('[data-import-result]');if(res){var arr=window._ugResults||[],idx=parseInt(res.dataset.importResult,10),draft=arr[idx];arr.forEach(function(r,j){if(r)r.selected=j===idx});renderImportResults('ug',arr,'');var p=SongImportDestinationController.importResultByIdOrUrl?SongImportDestinationController.importResultByIdOrUrl('ug',draft,songImportDestinationCtx()):(draft&&draft.chart?setImportedDraft('ug',draft):null);if(p&&p.then)p.then(function(){renderImportResults('ug',arr,'')});return}});var ugQuery=qs('ug-import-query'),ugUrl=qs('ug-source-url');if(ugQuery)ugQuery.addEventListener('keydown',function(e){if(e.key==='Enter')searchImport('ug')});if(ugUrl)ugUrl.addEventListener('keydown',function(e){if(e.key==='Enter')importFromUrl('ug')})}
var sop=qs('bs-sop-import');if(sop&&!sop.dataset.ready){sop.dataset.ready='1';sop.addEventListener('click',function(e){if(e.target===sop||e.target.id==='sop-import-cancel'){closeImportPages();return}if(e.target.id==='sop-import-search'){searchImport('sop');return}if(e.target.id==='sop-import-url-btn'){importFromUrl('sop');return}if(e.target.id==='sop-import-edit-btn'){editImported('sop');return}if(e.target.id==='sop-import-save-btn'){saveImported('sop');return}var res=e.target.closest('[data-import-result]');if(res){var arr=window._sopResults||[],idx=parseInt(res.dataset.importResult,10),draft=arr[idx];arr.forEach(function(r,j){if(r)r.selected=j===idx});renderImportResults('sop',arr,'');var p=SongImportDestinationController.importResultByIdOrUrl?SongImportDestinationController.importResultByIdOrUrl('sop',draft,songImportDestinationCtx()):(draft&&draft.chart?setImportedDraft('sop',draft):null);if(p&&p.then)p.then(function(){renderImportResults('sop',arr,'')});return}});var sopQuery=qs('sop-search-query'),sopUrl=qs('sop-source-url');if(sopQuery)sopQuery.addEventListener('keydown',function(e){if(e.key==='Enter')searchImport('sop')});if(sopUrl)sopUrl.addEventListener('keydown',function(e){if(e.key==='Enter')importFromUrl('sop')})}
...
function restoreFewerWarningContent(plan,selection){var msgs=restoreFewerMessages(plan,selection);if(!msgs.length)return '';var rows=msgs.map(function(m){var label=m.replace(/\s*\(.*$/,'');var nums=m.match(/\((.*?)\)/);return '<div class="backup-review-fewer-row"><span>'+esc(label)+'</span><strong>'+esc(nums?nums[1]:m)+'</strong></div>'}).join('');return '<div class="backup-review-fewer-title">Selected backup has fewer items</div><div class="backup-review-fewer-copy">Restoring selected sections may replace current data. A safety snapshot will be saved first.</div><div class="backup-review-fewer-table">'+rows+'</div>'}
...
function resolveSetKey(set,song){var owner=keyModel();if(owner&&owner.resolveSetKey)return owner.resolveSetKey(set,song,keyModelEnv());if(!set)return defaultKeyFor(song,null);set=ensureSetShape(set);var choice=normalizeKeyChoice(set.setKey||set.defaultKey||'original');if(choice==='original'||choice==='default')return defaultKeyFor(song,set);return resolveChoiceKey(choice,song,set)}
...
function resolveSetItemKey(set,item,song){var owner=keyModel();if(owner&&owner.resolveSetItemKey)return owner.resolveSetItemKey(set,item,song,keyModelEnv());if(itemHasExplicitKey(item))return resolveChoiceKey(item.setKey,song,set);return resolveSetKey(set,song)}
...
function resolveSongOpenKey(song,ctx){var set=findSongContextSet(ctx),item=ctx&&ctx.item?ctx.item:null;if(set&&!item&&ctx&&typeof ctx.index==='number')item=(set.items||[])[ctx.index]||null;if(set&&!item&&song&&song.id){item=(set.items||[]).find(function(it){return it&&(String(it.songId||it.id||'')===String(song.id));})||null;}return set?resolveSetItemKey(set,item,song):defaultKeyFor(song,null)}
...
function installSetListNotes(){personalSets.forEach(function(s){ensureSetShape(s);syncSetCounts(s)});persistSets();installSetDrag();var snSheet=qs('bs-set-notes');if(snSheet&&!snSheet.dataset.ready){snSheet.dataset.ready='1';snSheet.addEventListener('click',function(e){if(e.target===snSheet||e.target.id==='set-notes-cancel'||e.target.id==='set-notes-close')closeSheetElement(snSheet)});qs('set-notes-save').addEventListener('click',saveSetNotes)}var so=qs('bs-set-options');if(so&&!so.dataset.ready){so.dataset.ready='1';so.addEventListener('click',function(e){if(e.target===so){closeSetOptions();return}var a=e.target.closest('[data-set-action]');if(!a)return;e.preventDefault();e.stopPropagation();var act=a.dataset.setAction;closeSetOptions();setTimeout(function(){handleUnifiedSetOption(act)},0)});qs('set-options-done').addEventListener('click',closeSetOptions)}var sk=qs('bs-set-key');if(sk&&!sk.dataset.ready){sk.dataset.ready='1';sk.addEventListener('click',function(e){if(e.target===sk||e.target.id==='set-key-close'||e.target.id==='set-key-done')closeSetKeySheet();var opt=e.target.closest('[data-set-key]');if(opt)saveSetKey(opt.dataset.setKey)});}
...
function confirmDeletePersonalSetById(id){var doomed=personalSets.find(function(x){return x.id===id});return appConfirm('Delete this set list?',{title:'Delete set list',detail:doomed?('This set list has '+countLabel(doomed)+'. You can undo immediately after deleting.'):'You can undo immediately after deleting.',confirmText:'Delete',danger:true}).then(function(ok){if(!ok)return;var snap=dataSafetySnapshot('delete set list');personalSets=personalSets.filter(function(x){return x.id!==id});persistSets();renderPersonalHome();renderSettings();offerDataUndo('Set deleted',snap)});}
function personalSetActionCtx(){return {qs:qs,qsa:qsa,getSongs:function(){return songs||[]},getActiveSetId:function(){return activeSetId},getActiveSet:function(){return currentSet()},openSet:openSet,openPersonalNewSheet:openPersonalNewSheet,renderPersonalHome:renderPersonalHome,clearCurrentSet:clearCurrentSet,openSetSongPicker:openSetSongPicker,openSetOptions:openSetOptions,openSetNotes:openSetNotes,openSetKeySheet:openSetKeySheet,openWorkspacePublishSheet:function(){state.setOptionsContext='personal';return openWorkspacePublishSheet();},removeSetItem:removeSetItem,openSetItemEditor:openSetItemEditor,showSong:showSong,confirmDeletePersonalSetById:confirmDeletePersonalSetById};}
function bindPersonalSetActionControls(){if(window.WBPersonalSetController&&WBPersonalSetController.bindPersonalSetControls)return WBPersonalSetController.bindPersonalSetControls(personalSetActionCtx());return false;}
...
function updatePadNowPlayingPill(){var el=qs('pad-now-playing-pill'),wb=window.WorshipBasePad;if(!el||!wb||!wb.state){if(el){el.hidden=true;el.classList.remove('show')}return}var st=wb.state||{},on=!!st.on;if(!on){el.hidden=true;el.classList.remove('show');return}el.hidden=false;el.classList.add('show');var title=el.querySelector('[data-pad-now-title]'),sub=el.querySelector('[data-pad-now-sub]');var modeLabel=String(st.mode||'peaceful').charAt(0).toUpperCase()+String(st.mode||'peaceful').slice(1);var keyLabel=normalPadKey(st.key||state.displayKey||'C');if(title)title.textContent=modeLabel+' · '+keyLabel;if(sub)sub.textContent=''}
function ensurePadNowPlayingPill(){var el=qs('pad-now-playing-pill');if(el)return el;el=document.createElement('div');el.id='pad-now-playing-pill';el.className='pad-now-playing-pill';el.hidden=true;el.innerHTML='<div class="pad-now-copy"><strong data-pad-now-title>Peaceful · C</strong><span data-pad-now-sub></span></div><button type="button" data-pad-stop-global aria-label="Stop pads"><svg class="pad-stop-icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="7" y="7" width="10" height="10" rx="2"></rect></svg></button>';document.body.appendChild(el);el.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('[data-pad-stop-global]');if(!b)return;e.preventDefault();if(window.WorshipBasePad&&window.WorshipBasePad.stop)window.WorshipBasePad.stop();updatePadNowPlayingPill();renderPadSheet();updateSongTopActionStates();});return el}
function installPadNowPlayingPill(){ensurePadNowPlayingPill();var wb=window.WorshipBasePad;if(wb&&!wb._wbGlobalPillWrapped){['start','stop','setKey','setMode'].forEach(function(k){if(typeof wb[k]!=='function')return;var orig=wb[k];wb[k]=function(){var ret=orig.apply(wb,arguments);function sync(){updatePadNowPlayingPill();try{renderPadSheet()}catch(e){}try{updateSongTopActionStates()}catch(e){}};if(ret&&typeof ret.then==='function')ret.then(sync,sync);else setTimeout(sync,0);return ret};});wb._wbGlobalPillWrapped=true}updatePadNowPlayingPill()}
function handlePadSheetClick(e){var key=e.target.closest('[data-pad-key]'),mode=e.target.closest('[data-pad-mode]'),power=e.target.closest('[data-pad-power]');if(!window.WorshipBasePad)return;if(key){window.WorshipBasePad.setKey(key.dataset.padKey);renderPadSheet();updateSongTopActionStates();updatePadNowPlayingPill();return}if(mode){window.WorshipBasePad.setMode(mode.dataset.padMode);renderPadSheet();updateSongTopActionStates();updatePadNowPlayingPill();return}if(power){if(window.WorshipBasePad.state&&window.WorshipBasePad.state.on)window.WorshipBasePad.stop();else window.WorshipBasePad.start();renderPadSheet();updateSongTopActionStates();updatePadNowPlayingPill();return}}
...
function setPickerButton(id,value,sub){var b=qs(id);if(!b)return;var strong=b.querySelector('strong'),em=b.querySelector('em');if(strong)strong.textContent=value;if(em)em.textContent=sub||''}
...
function updateNnsConversion(){
  ...
  var baseKey=chordNnsState.transposeTarget||chordNnsState.key||'C';
  var converted=owner.convertProgression?owner.convertProgression(value,baseKey,chordNnsState.mode):null;
