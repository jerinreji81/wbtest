#!/usr/bin/env node
/**
 * WorshipBase RC3 release preflight.
...
if (exists(path.join(DIST, 'index.html'))) {
  ...
  if (!html.includes('Rebuild_8.17RC3')) errors.push('Dist index does not contain Rebuild_8.17RC3 version label');
...
if (exists(path.join(DIST, 'wb-offline-sw.js'))) {
  ...
  if (!sw.includes('rc3')) errors.push('Service worker cache version does not contain rc3');
...
if (manifest) {
  if (manifest.phase !== 'RC3') errors.push(`Manifest phase is ${manifest.phase}, expected RC3`);
...
console.log('WorshipBase RC3 release preflight');
