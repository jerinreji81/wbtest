#!/usr/bin/env node
/**
 * WorshipBase RC1 release preflight.
...
if (exists(path.join(DIST, 'index.html'))) {
  ...
  if (!html.includes('Rebuild_8.17RC1')) errors.push('Dist index does not contain Rebuild_8.17RC1 version label');
...
if (exists(path.join(DIST, 'wb-offline-sw.js'))) {
  ...
  if (!sw.includes('rc1')) errors.push('Service worker cache version does not contain rc1');
...
if (manifest) {
  if (manifest.phase !== 'RC1') errors.push(`Manifest phase is ${manifest.phase}, expected RC1`);
...
console.log('WorshipBase RC1 release preflight');
