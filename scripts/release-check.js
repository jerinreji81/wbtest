#!/usr/bin/env node
/**
 * WorshipBase Phase L1 release preflight.
...
if (exists(path.join(DIST, 'index.html'))) {
  ...
  if (!html.includes('Rebuild_8.17L1')) errors.push('Dist index does not contain Rebuild_8.17L1 version label');
...
if (exists(path.join(DIST, 'wb-offline-sw.js'))) {
  ...
  if (!sw.includes('phase-l1')) errors.push('Service worker cache version does not contain phase-l1');
...
if (manifest) {
  if (manifest.phase !== 'L1') errors.push(`Manifest phase is ${manifest.phase}, expected L1`);
...
console.log('WorshipBase Phase L1 release preflight');
